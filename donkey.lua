...
collectgarbage(  )
print( string.format( 'Train loader serves %d actions of %d videos with %s classes.',
trainLoader.aid2vid:numel(  ), trainLoader.vid2path:size( 1 ), trainLoader.cid2name:size( 1 ) ) )
...
collectgarbage(  )
print( string.format( 'Test loader serves %d actions of %d videos with %s classes.',
testLoader.aid2vid:numel(  ), testLoader.vid2path:size( 1 ), testLoader.cid2name:size( 1 ) ) )
...
if paths.filep( opt.pathImStat ) then
	...
	for v = 1, numVideos do
		local img = trainLoader:sampleActions( 1, 1 )[ 1 ]
		for ch = 1, 3 do
